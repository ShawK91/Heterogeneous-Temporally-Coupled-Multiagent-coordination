...
def evolve(gridworld, parameters, best_hof_score):
    ...
    for i in range(parameters.num_agents_executioner): gridworld.agent_list_executioner[i].evo_net.referesh_genome_list()

    #MAKE SELECTION POOLS
    teams = np.zeros(parameters.num_agents_scout + parameters.num_agents_executioner).astype(int) #Team definitions by index
    ...
    for i in range(parameters.num_agents_executioner): #Filling the selection pool
        if parameters.use_neat: ig_num_individuals = len(gridworld.agent_list_executioner[i].evo_net.genome_list) #NEAT's number of individuals can change
        else: ig_num_individuals = parameters.population_size #For keras_evo-net the number of individuals stays constant at population size
        selection_pool.append(np.arange(ig_num_individuals))
    ...
    for evals in range(parameters.population_size * num_evals): #For all evaluation cycles
        ...
        for i in range(len(teams)):
            if i < parameters.num_agents_scout: #Scouts
                ...
                gridworld.agent_list_executioner[i-parameters.num_agents_scout].evo_net.build_net(teams[i])  # build network for the genomes within the team
    ...
    for agent in gridworld.agent_list_executioner:
        agent.evo_net.update_fitness()# Assign fitness to genomes #
        agent.evo_net.epoch() #Run Epoch update in the population
        if parameters.baldwin and parameters.update_sim and not parameters.share_sim_subpop: agent.evo_net.bald.port_best_sim()  # Deep copy best simulator
...
def run_simulation(parameters, gridworld, teams): #Run simulation given a team and return fitness for each individuals in that team
    ...
    for steps in range(parameters.total_steps):  # One training episode till goal is not reached
        ...
        for id, agent in enumerate(gridworld.agent_list_executioner):  #get all the action choices from the agents
        ...
        for id, agent in enumerate(gridworld.agent_list_executioner): agent.referesh(teams[id],
